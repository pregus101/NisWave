...
class SongBar:
    ...
    def draw(self) -> None:
        if self.total_length > 0:
            ...
            text_surface: Surface = Font.render(f"{int(self.current_length//60)}:{int(self.current_length%60):02d} / {int(self.total_length//60)}:{int(self.total_length%60):02d}", True, (255, 255, 255))
    ...
    def adjust_time(self, mouse_position: list[float]) -> None:
        ...
        relative_x: float = mouse_position[0] - (((self.screen_width-self.screen_width/5)/2+self.screen_width/5)-self.bar_width_hight/2)
        relative_x = max(-1, min(relative_x, self.bar_width_hight+1))  # Clamp to bar width
        relative_y: float = mouse_position[1] - ((self.screen_height/2+(120*((self.screen_width/1920 + self.screen_height/1147) / 2)))+self.bar_width_hight/2)

        if 0 <= relative_y <= 6 and 0 < relative_x < self.bar_width_hight:  # Only adjust if mouse is within the bar height
            new_current_length: float = (relative_x / self.bar_width_hight) * self.total_length
            if self.visualizer:
